...
- Logic dữ liệu tách khá rõ giữa board, input, merge, goal và rendering [query].
- Merge theo `Rect` runtime phản ánh đúng shape sau khi jelly đổi layout [query].
- Cascade resolve trong cùng lượt phù hợp với rule “ăn tiếp ngay sau khi phình ra” [query].

## Chỗ nên tối ưu

### 1. `JellyPieceView.Render()` đang tốn nhất

Hiện tại mỗi lần render, hàm này `Destroy` toàn bộ `SpriteRenderer` cũ rồi `Instantiate` lại sprite mới. Trên mobile, đây là kiểu cấp phát/giải phóng object rất dễ tạo GC spike và frame drop; object pooling hoặc reuse object là cách tối ưu phù hợp hơn [3][2].

**Nên làm:** tạo sẵn tối đa 4 `SpriteRenderer`, sau đó chỉ `SetActive`, đổi `localPosition`, `localScale`, `color` khi refresh [2][4].

### 2. `JellyPopEffect` nên dùng pool

`SpawnPopEffect()` đang instantiate effect mới mỗi lần collect xong rồi destroy sau đó. Với effect lặp lại liên tục trong game mobile, pooling giúp giảm allocation và chi phí tạo/hủy object đáng kể [3][5].

### 3. `BuildMatchStateKey()` tạo nhiều string

Mỗi pass resolve đang tạo list string, sort, rồi `string.Join`. Điều này an toàn cho logic nhưng tạo rác bộ nhớ. Nếu profiler cho thấy GC ở đây, nên thay bằng hash số nguyên hoặc reusable buffer [1].

### 4. `Physics2D.OverlapPointAll()` hơi rộng

`InputHandler.TrySelectPiece()` dùng `OverlapPointAll` rồi tự lọc `JellyPiece`. Nếu scene đơn giản thì vẫn chạy được, nhưng trên mobile nên dùng `LayerMask` riêng cho draggable preview và ưu tiên `OverlapPoint` hoặc raycast gọn hơn để giảm số collider phải duyệt [1].

### 5. Có thể cache collection tạm

Các list/hashset như `matchedSubs`, `touchedPieces`, `emptiedPieces`, `survivedPieces` được tạo nhiều lần trong resolve chain. Board nhỏ thì chưa đáng lo, nhưng nếu về sau board lớn hơn hoặc animation dày hơn, nên cân nhắc reuse collection để giảm GC [1].

## Chỗ có thể xóa nếu không dùng

### Có thể xóa ngay nếu project không dùng nơi khác

- `BoardManager.GetAllFilledCells()` — chưa xuất hiện trong flow code hiện tại [query].
- `BoardManager.GetNeighbors()` — hiện chưa được `GameManager` hay `MergeSystem` dùng [query].

### Có thể giản lược

- `MatchedSubCellData.slot` — merge hiện tại không còn dùng slot để quyết định va chạm; nếu chỉ để debug nhẹ thì giữ, còn không có thể xóa [query].
- `JellySlot` — vẫn hữu ích cho debug tên layout, nhưng không còn là nguồn quyết định merge; nếu bỏ debug slot-based thì có thể giảm vai trò đáng kể [query].
- constructor bị comment trong `JellySubCell` có thể bỏ hẳn nếu không có save/load hoặc clone logic [query].

### Có thể xem lại

`BoardManager.Awake()` gọi `InitBoard()`, sau đó `GameManager.Start()` lại `ConfigureBoard(...)` và init lần nữa. Điều này không sai nhưng hơi dư; nếu board luôn phụ thuộc level data, có thể chỉ init sau khi cấu hình level [query].

## Cleanup đề xuất

1. Giữ lại kiến trúc hiện tại vì flow gameplay đã đúng hướng.
2. Viết lại `JellyPieceView` theo kiểu reuse 4 renderers.
3. Pool `JellyPopEffect`.
4. Xóa các API chưa dùng như `GetAllFilledCells()` và `GetNeighbors()` nếu không phục vụ debug/editor tooling.
5. Giảm các field trung gian không còn cần cho merge slot-based.
...
Spawn preview jelly
-> Random subcells
-> Render layout nội bộ
-> Player drag & drop vào ô trống
-> Board gắn piece vào cell
-> ResolveTurn bắt đầu
-> Check 4 neighbor pieces
-> So từng cặp subcell cùng màu bằng localRect
-> Match thì remove subcell
-> RefreshVisual jelly còn sống
-> Collect goal
-> Piece rỗng bay đi và bị xóa
-> Piece còn sống + hàng xóm được queue check tiếp
-> Hết chain thì spawn jelly mới / win / lose
```

## Gợi ý bước tiếp theo

Mức ưu tiên kỹ thuật cao nhất hiện nay là đổi `JellyPieceView` sang **không Instantiate/Destroy mỗi lần RefreshVisual**, vì đây là điểm có khả năng gây tụt hiệu năng rõ nhất trên Android trong flow hiện tại [3][2].
